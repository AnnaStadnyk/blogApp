...
onBeforeMount(async () => {
  post = await articleStore.getPost(props.category, props.id)
...
function onFormSubmit() {
  ...
  try {
    ...
    articleStore.editPost(props.category, props.id, newPost, selectedFiles.value)
    ...
    <div class="wrapper-content">
      <form action="" class="form" enctype="multipart/form-data" @submit.prevent="onFormSubmit">
        ...
        <div class="form__item">
          ...
          <Transition>
            <span v-show="selectedFiles.length > 0 || post.images.length > 0"
              ><i class="fa-regular fa-file"></i
              ><span>{{
                selectedFiles.length > 0 ? selectedFiles.length : post.images.length
              }}</span></span>
          </Transition
          >
        </div>

        <TheButton mode="inverce">Edit Post</TheButton>
      </form>
    </div>
  </section>
</template>
